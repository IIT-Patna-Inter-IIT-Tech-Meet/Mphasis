...
def get_avilable_seats(aircraft, flight):
...
def get_one_hop_flights(main_flight, time_threshold):
    ...
    for flight in conn_flights:
        ...
        avilaible_seats, total_avilable_seats = get_avilable_seats(schedule.aircraft_id, flight.f2_id)
...
def get_two_hop_flights(main_flight, time_threshold):
    query = f"""
        select 
            *,
    ...
    for flight in conn_flights:
        ...
        avilaible_seats, total_avilable_seats = get_avilable_seats(schedule.aircraft_id, flight.f3_id)
...
def util_flight_ranking(flight_id, max_hop = 2):
    ...
    c_data = get_one_hop_flights(main_flight, time_threshold) if max_hop >= 1 else []
    t_data = get_two_hop_flights(main_flight, time_threshold) if max_hop >= 2 else []

    return {"data": data, "r_flights": r_flights, "c_flights": c_data, "t_flights": t_data}
