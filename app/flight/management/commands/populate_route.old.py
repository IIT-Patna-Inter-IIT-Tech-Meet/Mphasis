...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for data in loadjson():
            ...
            try:
                sources = data["result"]["response"]["airport"]["pluginData"][
                    "schedule"
                ]["arrivals"]["data"]
            except:
                sources = []
            ...
            for source in sources:
                ...
                try:
                    ...
                    try:
                        owner_code = (
                            f'{source["flight"]["owner"]["code"]["iata"]} +  {source["flight"]["owner"]["code"]["icao"]}',
                        )
                ...
                try:
                    ...
                    try:
                        status_json = source["flight"]["status"]["generic"]["status"][
                            "color"
                        ]
                    ...
                    flight = Flight.objects.create(
                        id=flight_id,
                        flight_number=source["flight"]["identification"]["number"][
                            "default"
                        ],
                        aircraft_id=aircraft,
                        ...
                        departure_time=datetime.fromtimestamp(
                            source["flight"]["time"]["scheduled"]["departure"],
                            tz=pytz.timezone(TIME_ZONE),
                        ),
                        arrival_time=datetime.fromtimestamp(
                            source["flight"]["time"]["scheduled"]["arrival"],
                            tz=pytz.timezone(TIME_ZONE),
                        ),
                        flight_time=int(
                            source["flight"]["time"]["scheduled"]["arrival"]
                        )
                        - int(source["flight"]["time"]["scheduled"]["departure"]),
                        status=status_json,
                    )
                    flight.save()
