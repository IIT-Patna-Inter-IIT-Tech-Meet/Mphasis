...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for aircraft in all_aircrafts:
            count = FLIGHT_COUNT_PER_AIRCRAFT + random.randint(
                -1 * FLIGHT_COUNT_MARGIN, FLIGHT_COUNT_MARGIN
            )
            for i in range(count):
                ...
                time_delta = datetime.timedelta(
                    hours=random.randint(0, 2), minutes=random.randint(1, 60)
                )
                arrival_time = departure_time + time_delta

                start_date = datetime.datetime.now(
                    pytz.timezone(TIME_ZONE)
                ).date() + datetime.timedelta(days=random.randint(-15, 15))
                ...
                for j in range(random.randint(3, 7)):
                    ...
                    d = timezone.make_aware(
                        datetime.datetime.combine(date, departure_time.time())
                    )
                    a = d + time_delta
        ...
        print(
            f"Populating...{len(flights)} flights,{len(flights_schedule)} flights schedule,{len(flights_schedule_date)} flights schedule date "
        )
