from flight.core.P2Q import P2Q
...
class Grover:
    ...
    def Grover_search(self, states):
        if len(states) == 1:
            ...
            def grover(
                oracle: qc = oracle(),
                diffuser: qc = diffuser(),
                name: str = "Grover Circuit",
                display_grover: bool = True,
            ) -> qc:
            ...
            def outcome(winners: dict[str, int]) -> None:
                ...
                print(
                    f"Target(s) found with {sum(winners.values()) / SHOTS:.2%} accuracy!\n"
                    if all(winner in TARGETS for winner in [*winners])
                    else "Target(s) not found...\n"
                )

            def display_results(
                results: dict[str, int], combine_other_states: bool = True
            ) -> None:
                # State(s) with highest count and their frequencies
                winners = {
                    winner: results.get(winner)
                    for winner in nlargest(len(TARGETS), results, key=results.get)
                }
                ...
                others = {
                    state: frequency
                    for state, frequency in results.items()
                ...
                other_states_y_axis = (
                    [sum([*others.values()])]
                    if combine_other_states
                    else [*others.values()]
                )

                # Create histogram for simulation results
                figure, axes = subplots(
                    num="Grover's Algorithm — Results", layout="constrained"
                )
                ...
                axes.set_title(
                    f"Outcome of {SHOTS} Simulations", fontsize=int(FONTSIZE * 1.45)
                )
                ...
                xticks(
                    fontsize=FONTSIZE,
                    family="monospace",
                    rotation=0 if combine_other_states else 70,
                )
            ...
            results = execute(
                grover_circuit, backend=Aer.get_backend("qasm_simulator"), shots=SHOTS
            ).result()
